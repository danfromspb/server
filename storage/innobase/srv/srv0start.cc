...
{
  auto path = get_log_file_path("ib_logfile").append(suffix);
...
static dberr_t create_log_file(lsn_t lsn, std::string& logfile0)
{
	...
	logfile0 = get_log_file_path("ib_logfile").append(INIT_LOG_FILE0);
	...
	log_sys.log.set_file_name(logfile0);
...
static dberr_t create_log_file_rename(lsn_t lsn, std::string &logfile0)
{
  ...
  logfile0= new_name;
  log_sys.log.set_file_name(new_name);
  log_mutex_exit();
...
dberr_t find_and_check_log_file(bool &log_file_found)
{
  ...
  Mariabackup --prepare would create an empty ib_logfile0. Tolerate it. */
...
dberr_t srv_start(bool create_new_db)
{
	...
	if (create_new_db) {
		...
		log_sys.log.set_file_name(get_log_file_path());
		log_sys.log.open_file();

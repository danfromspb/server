					...
					dict_index_t* index, mtr_t* mtr)
{
	...
	if (trailer_size) {
		mtr->zmemcpy(block->page, page_zip_get_size(page_zip)
                         ...
                         const byte *free, mtr_t *mtr)
{
  ...
  byte *page_free= static_cast<byte*>
    (MY_ASSUME_ALIGNED(PAGE_FREE + PAGE_HEADER + block->frame, 2));
  mach_write_to_2(page_free, page_offset(rec));
  byte *garbage= static_cast<byte*>
    (MY_ASSUME_ALIGNED(PAGE_GARBAGE + PAGE_HEADER + block->frame, 2));
  ...
  byte *page_n_recs= static_cast<byte*>
    (MY_ASSUME_ALIGNED(PAGE_N_RECS + PAGE_HEADER + block->frame, 2));

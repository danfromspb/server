                                 ...
                                 ulint *heap_no, mtr_t *mtr)
{
  ...
  byte *heap_top= static_cast<byte*>
   (MY_ASSUME_ALIGNED(PAGE_HEAP_TOP + PAGE_HEADER + block->frame, 2));
  ...
  byte *n_heap= static_cast<byte*>
   (MY_ASSUME_ALIGNED(PAGE_N_HEAP + PAGE_HEADER + block->frame, 2));
	...
	mtr_t*		mtr)	/*!< in/out: mini-transaction */
{
	...
	if (UNIV_LIKELY_NULL(free_rec)) {
		...
		byte* page_free = static_cast<byte*>
			(MY_ASSUME_ALIGNED(PAGE_FREE + PAGE_HEADER
					   + block->frame, 2));
		byte* page_garbage = static_cast<byte*>
			(MY_ASSUME_ALIGNED(PAGE_GARBAGE + PAGE_HEADER
					   + block->frame, 2));
                                         ...
                                         const dict_index_t *index, mtr_t *mtr)
{
	...
	ulint n_dense = page_dir_get_n_heap(page_zip->data)
	...
	if (!page_is_leaf(page_zip->data)) {
		...
		ut_ad(!memcmp(dst, field_ref_zero,
	...
	if (const ulint len = ulint(dir - stored)) {
		...
		memmove_aligned<2>(dst, stored, len);
		/* TODO: write MEMMOVE record */
		mtr->zmemcpy(block->page, dst - page_zip->data, len);
	}
}

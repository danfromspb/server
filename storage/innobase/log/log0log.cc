...
log_set_capacity(ulonglong file_size)
{
	...
	if (free >= smallest_capacity / 2) {
		ib::error() << "Cannot continue operation. ib_logfile0 is too"
			" small for innodb_thread_concurrency="
			<< srv_thread_concurrency << ". The size of"
			" ib_logfile0 should be bigger than"
...
void log_t::file::set_file_name(std::string name)
{
  file_name= std::move(name);
}

void log_t::file::open_file()
{
  ut_ad(file == OS_FILE_CLOSED);
  bool success;
  file= os_file_create(innodb_log_file_key, file_name.c_str(),
                       OS_FILE_OPEN | OS_FILE_ON_ERROR_NO_EXIT, OS_FILE_NORMAL,
                       OS_LOG_FILE, srv_read_only_mode, &success);
  if (!success)
  {
    ib::fatal() << "os_file_create(" << file_name << ") failed";
  }
}

void log_t::file::read(size_t offset, span<byte> buf)
{
  ut_ad(file != OS_FILE_CLOSED);

  if (const dberr_t err=
          os_file_read(IORequestRead, file, buf.data(), offset, buf.size()))
  {
    ib::fatal() << "os_file_read(" << file_name << ") returned " << err;
  }
}

void log_t::file::write(size_t offset, span<byte> buf)
{
  ut_ad(file != OS_FILE_CLOSED);

  if (const dberr_t err= os_file_write(IORequestWrite, file_name.c_str(), file,
                                       buf.data(), offset, buf.size()))
  {
    ib::fatal() << "os_file_write(" << file_name << ") returned " << err;
  }
}

void log_t::file::fsync()
{
  ut_ad(file != OS_FILE_CLOSED);

  log_sys.pending_flushes.fetch_add(1, std::memory_order_acquire);
  if (!os_file_flush(file))
    ib::fatal() << "os_file_flush(" << file_name << ") failed";
  log_sys.pending_flushes.fetch_sub(1, std::memory_order_release);
  log_sys.flushes.fetch_add(1, std::memory_order_release);
}

void log_t::file::fdatasync()
{
  ut_ad(file != OS_FILE_CLOSED);

  log_sys.pending_flushes.fetch_add(1, std::memory_order_acquire);
  if (!os_file_flush_data(file))
    ib::fatal() << "os_file_flush_data(" << file_name << ") failed";
  log_sys.pending_flushes.fetch_sub(1, std::memory_order_release);
  log_sys.flushes.fetch_add(1, std::memory_order_release);
}
...
void log_t::file::close_file()
{
  ...
  if (!os_file_close(file))
    ib::fatal() << "os_file_close(" << file_name << ") failed";
  file= OS_FILE_CLOSED;
}

...
/* As long as fil_io() is used to handle log io, log max size is limited
...
struct log_t{
  ...
  private:
    ...
    pfs_os_file_t file{OS_FILE_CLOSED};
  ...
  public:
    ...
    /** simple setter, does not close or open log file */
    ...
    void read(size_t offset, span<byte> buf);
    /** writes buffer to log file
    @param[in]	offset		offset in log file
    @param[in]	buf		buffer from which to write */
    void write(size_t offset, span<byte> buf);
    /** flushes OS page cache for log file */
    void fsync();
